...
| Change output configuration | Experimental output schema, codec, authenticated service and live configuration owner; independent lifecycle proof remains | Output authority/Session, not a shell permission implied by output facts | t023; no new generic settings UI required |

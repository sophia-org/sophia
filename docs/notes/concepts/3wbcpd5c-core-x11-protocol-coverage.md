...
| 83 | ListInstalledColormaps | Serve it | The reply is the one installed colormap, the default: setup advertises one installed map and GetWindowAttributes reports it installed. With t169 the rest of the family got the protocol's framing, so a request one unit off is BadLength first. CopyColormapAndFree creates on the source's visual and, since t212, moves the requesting client's RGB component references. FreeColors releases those references even though the TrueColor palette itself is immutable. |

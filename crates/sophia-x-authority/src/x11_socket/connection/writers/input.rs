        ...
        while !writer_stop.load(Ordering::Acquire) {
            ...
            let (delivered_window, pointer_surface_window, pointer_event_ancestry) = match event {
                ...
                XAuthorityInputEvent::Pointer(pointer) => {
                    ...
                    // hears the motion.
                    let event_window = if selections.surface_contains(surface_window, pointer.event_x, pointer.event_y) {
                        selections.pointer_event_target(surface_window, pointer.event_x, pointer.event_y)
                    } else {
            ...
            // The window the pointer is in, this table's view: the crossing
            // events are the moves of this, not of the window the event is
            // reported on.
            let pointer_window = pointer_event_ancestry
                .as_ref()
            ...
            let mut xi_source_records = if let (
                ...
                encode_xi_source_pointer_events(X11XiSourceEvent {
                    ...
                    ancestry,
                    pointer,
                    crossing: (pointer_sent_to != pointer_window)
                        .then_some((pointer_sent_to, pointer_window.unwrap_or(delivered_window))),
                })
            } else {
                [None, None, None, None]
            };
            let write_result = (|| -> Result<X11InputWriteOutcome, X11SetupSocketError> {
                ...
                if let Some((previous, to, out_type, in_type)) = transition {
                    write_x11_pointer_crossings(X11PointerCrossingWrite {
                        ...
                        namespace,
                        core_event_selections: &core_event_selections,
                        focused_surface_window: &focused_surface_window,
                        root,
                        xi_pointer_crossing_mask,
                    })?;
                    pointer_sent_to = Some(to);
                }

...
impl PrivateSettlementOwner {
    ...
    fn reserve_ordered_continuation(
        ...
        // AND THE DESTINATION ITS MAINTENANCE WILL NEED, on this same credit
        // and in this same acquisition.
        //
        // BEFORE THIS CONNECTION IS PUBLISHED, which is the whole point of
        // doing it here. What will eventually fill it -- a joined worker, a
        // closed gate, an obligation somebody commits -- all happens long
        // afterwards, and finding storage for it then would mean a connection
        // whose row was already reachable could be left unable to be finished
        // for want of room. One place, one destination, on the credit this
        // reservation already charged: no second pool, no second charge.
        //
        // INERT. Reserving somewhere for an obligation is not committing one,
        // and nothing about this authorises anything to drive anything.
        while held.holders.len() <= index {
            held.holders.push(PrivateHolderPlace::Free);
        }
        held.holders[index] = PrivateHolderPlace::Reserved(index);
        drop(held);
    ...
    fn return_ordered_continuation(&self, index: usize, record: &Arc<PrivateOrderedHome>) {
        ...
        Self::release_maintenance_destination(&mut held, index);

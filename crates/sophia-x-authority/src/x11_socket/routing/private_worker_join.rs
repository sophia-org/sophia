...
enum PrivateExitReading {
    ...
    ///
    /// EXACTLY THAT, AND NOT A PANIC. A body between its two writes looks the
    /// same as one that unwound; what says which is the join.
    Unclassified,
...
#[cfg(unix)]
#[cfg_attr(not(test), allow(dead_code))] // Read by a caller no production site has yet.
impl PrivateReapingRecord {
    /// A record for a join nobody has asked for yet.
    fn unasked() -> Self {
        Self {
...
fn reap_connection_worker(
    slot: &Mutex<PrivateWorkerSlot>,
    exit: &PrivateWorkerExit,
    record: &PrivateReapingRecord,
) -> PrivateReaping {
    ...
    record.phase.store(1, Ordering::Release);

    let handoff = hand_worker_to_joiner(slot);
    ...
    let Some(handle) = handoff.handle else {
        ...
        record.phase.store(0, Ordering::Release);
    ...
    let result = match joined {
        ...
        Err(payload) => PrivateJoinResult::Panicked(payload),
    };
    let retained = record.result.set(result).is_ok();
    ...
    record.phase.store(2, Ordering::Release);

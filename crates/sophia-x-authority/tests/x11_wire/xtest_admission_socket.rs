...
mod xtest_admission_socket {
    ...
    impl XtestClient {
        ...
        pub(super) fn fake_input_at(&mut self, event_type: u8, detail: u8, x: i16, y: i16) {
            let mut body = [0u8; 32];
            body[0] = event_type;
            body[1] = detail;
        ...
        pub(super) fn next_event(&mut self, code: u8) -> [u8; 32] {
            loop {
        ...
        pub(super) fn fake_input(&mut self, event_type: u8, detail: u8) {
            let mut body = [0u8; 32];
            body[0] = event_type;
            body[1] = detail;
            self.stream
        ...
        pub(super) fn assert_quiet(&mut self, label: &str) {
            let mut request = vec![43, 0];
            push_u16(&mut request, self.order, 1);
            self.stream.write_all(&request).unwrap();

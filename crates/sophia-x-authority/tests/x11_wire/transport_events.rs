...
#[test]
fn x11_property_notify_carries_the_requests_server_time() {
    let namespace = NamespaceId::from_raw(55);
    let mut runtime = XAuthorityRuntime::new();
    let mut atoms = XAtomTable::new();
    let mut properties = XPropertyTable::new();

    // The suite's own gettime() helper reads the server time out of a
    // PropertyNotify. Zero is the one value a server may never generate --
    // it is CurrentTime on the wire -- so a zero here leaves a client with
    // no way to learn the time at all.
    let request = change_property_request(
    ...
    .unwrap();
    let mut dispatch = dispatch_context(namespace, 1, XByteOrder::LittleEndian, 18);
    dispatch.server_time = 91_537;
    ...
    let notify = encoded
        .iter()
        .find(|output| output[0] == 28)
        .expect("a stored property notifies its watchers");
    assert_eq!(
        read_u32(XByteOrder::LittleEndian, &notify[12..16]),
        91_537,
        "the notification carries the time the request was served at"
    );
}

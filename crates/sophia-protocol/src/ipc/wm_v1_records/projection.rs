pub fn encode_wm_v1_policy_projection(
    ...
    chunks.extend(super::encode_wm_overview(
        &proposal.overview_workspaces,
        proposal.connection_epoch,
        chunks.len() as u16,
    )?);
...
pub fn decode_wm_v1_policy_projection(
    ...
    for (ordinal, chunk) in transfer.chunks.iter().enumerate() {
        ...
        match chunk.record_kind {
            ...
            | super::PROJECTION_OVERVIEW_WORKSPACE_RECORD_KIND
            | super::PROJECTION_OVERVIEW_PLACEMENT_RECORD_KIND
    ...
    Ok(PolicyProjectionProposal {
        overview_workspaces: super::decode_wm_overview(&transfer.chunks)?,

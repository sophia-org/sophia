...
fn retain_existing_surface(layout: &mut PersistentLiveLayout) {
    let geometry = Rect { x: 30, y: 30, width: 160, height: 120 };
    let size = Size { width: geometry.width, height: geometry.height };
    ...
    batch.surface_routes.push(sophia_x_authority::XAuthoritySurfaceRouteObservation {
        surface: SURFACE, client, admission: None,
    });
    batch.surface_presentations.push(sophia_x_authority::XAuthoritySurfacePresentationObservation {
        surface: SURFACE,
        ...
        mapped: true,
        geometry,
        constraints: SurfaceConstraints { min_size: None, max_size: None },
        generation: 8,
    });
    ...
    layout.layers.insert(SURFACE, LayerSnapshot {
        surface: SURFACE,
    ...
    assert!(layout.admissions.begin_control(SURFACE, TransactionId::from_raw(700), geometry));
    assert!(layout.admissions.acknowledge_control(SURFACE, TransactionId::from_raw(700)));
    assert!(layout.admissions.mark_managed(SURFACE));
}
...
fn next_proposal(
    ...
    loop {
        ...
        assert!(!wm.force_transport_restart && !wm.degraded, "same Hagia must remain admitted");
...
fn matching_pixels(layout: &mut PersistentLiveLayout) {
    ...
    let geometry = pending.layers.iter().find(|layer| layer.surface == SURFACE).unwrap().geometry;
...
fn acknowledge_frontend_controls(layout: &mut PersistentLiveLayout, controls: &mut crate::session_control::SessionControlQueue) {
    use sophia_x_authority::{XAuthorityClientControlAck, XAuthorityControlAck, XAuthorityControlKind, XAuthorityControlOutcome};
    let (sender, receiver) = std::sync::mpsc::sync_channel(32);
    let (ack_sender, ack_receiver) = std::sync::mpsc::sync_channel(32);
    let mut completions = Vec::new();
    controls.service(&sender, &ack_receiver, Instant::now(), &mut completions).unwrap();
    assert!(completions.is_empty());
    let commands = receiver.try_iter().collect::<Vec<_>>();
    assert!(!commands.is_empty());
    for command in &commands {
        ack_sender.send(XAuthorityClientControlAck {
    ...
    for completion in completions {
        ...
        if completion.key.kind == XAuthorityControlKind::SetPresentationState {
            assert!(layout.acknowledge_presentation_control(completion.key.transaction, completion.key.surface));
        }
    }
}

fn await_ready(wm: &mut LiveWmSession, layout: &mut PersistentLiveLayout, output: sophia_engine::HeadlessOutput) {
    let deadline = Instant::now() + Duration::from_secs(5);
    loop {
        assert!(wm.poll_public_request(layout, output, false).unwrap().is_none());
        let public = wm.public.as_ref().unwrap();
        assert!(public.pending_operation.is_none(), "failed action must not issue an operation");
        if public.transport_ready && public.in_flight_request.is_none() { return; }
        assert!(!wm.force_transport_restart && !wm.degraded);
        assert!(Instant::now() < deadline, "same child outcome/Ready deadline");
        std::thread::sleep(Duration::from_millis(2));
    }
}
    ...
    loop {
        match std::fs::File::open(path) {
            Ok(mut file) => {
                ...
                assert!(!bytes.is_empty(), "atomically published checkpoint is complete");
                return (bytes, (metadata.dev(), metadata.ino()));
            }
            Err(error) if error.kind() == std::io::ErrorKind::NotFound => {
                assert!(Instant::now() < deadline, "Hagia checkpoint consumption deadline: {error}");
                std::thread::sleep(Duration::from_millis(2));
            }
...
fn normal_hagia_held_resize_commits_then_answers_a_fresh_request() {
    with_normal_hagia("held-resize-commit", |wm, layout, _, output, checkpoint, identity| {
        ...
        assert!(!proposal.requested_sizes.is_empty(), "resize must come from Hagia");
        ...
        assert_eq!(wm.public.as_ref().unwrap().reducer.committed(), old_projection);
        assert!(!checkpoint.exists());
        acknowledge_frontend_controls(layout, &mut controls);
        assert!(!layout.pending_is_ready());
        assert_eq!(layout.layers[&SURFACE], old_layer);
        matching_pixels(layout);
        assert!(!layout.pending.as_ref().unwrap().staged_transactions.is_empty());
        assert!(layout.pending_is_ready());
        assert!(wm.prepare_public_layout_commit(layout).unwrap());
        let result = layout.resolve_pending().expect("matching pixels resolve real pending layout");
        ...
        assert_ne!(wm.public.as_ref().unwrap().reducer.committed(), old_projection);
        wm.enqueue_relayout(layout, output).unwrap();
        ...
        assert!(checkpoint.exists(), "real Hagia committed outcome persists its checkpoint before next answer");
...
fn normal_hagia_timed_out_session_action_keeps_checkpoint_and_answers_next_request() {
    with_normal_hagia("held-resize-timeout", |wm, layout, _, output, checkpoint, identity| {
        let peer = wm.supervisor.peer_id();
        retain_existing_surface(layout);
        wm.enqueue_relayout(layout, output).unwrap();
        ...
        assert!(layout.pending_is_ready());
        assert!(wm.prepare_public_layout_commit(layout).unwrap());
        let result = layout.resolve_pending().expect("initial actual held resize ready");
        assert_eq!(result.update.commit.outcome, TransactionOutcome::Committed);
        ...
        assert!(wm.public.as_ref().unwrap().session_operations.iter().any(|operation| operation.slot == 1));
        let action = wm.public.as_ref().unwrap().actions.iter()
            .find(|entry| entry.session_operation_slot == Some(1))
            .expect("real profile advertises slot one").action;
        ...
        assert!(wm.set_shell_reservation_bands(vec![OutputReservation {
            edge: OutputEdge::Top, depth: 80,
            span: AxisSpan { start: bounds.x, end: bounds.x + bounds.width },
        }]));
        wm.update_output_work_areas(layout, &[output], output).unwrap();
        let proposal = next_proposal(wm, layout, output);
        assert_eq!(proposal.source, Some(LiveWmProposalSource::Action(action)));
        let settlement = proposal.policy_settlement.unwrap();
        assert!(settlement.expect_session_operation, "must exercise P1 operation-slot boundary");
        assert!(!proposal.requested_sizes.is_empty(), "Hagia must compute the changed target");
        ...
        assert_eq!(wm.public.as_ref().unwrap().reducer.committed(), projection_before);
        assert!(wm.public.as_ref().unwrap().prepared.is_none());
        assert!(layout.force_pending_timeout());
        let result = layout.expire_pending(&mut controls).unwrap().expect("actual pending expiry");

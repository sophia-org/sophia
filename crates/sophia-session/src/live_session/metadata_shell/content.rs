...
mod placement;
mod projection;
use projection::project_render_bundle;
#[path = "content/submission.rs"]
mod submission;
pub use native_service::NativeLauncherContentService;
use placement::{panel_pixel_thickness, panel_rect, popout_rect, quantize};
...
impl LiveContentSession {
    ...
    fn resolve_allocation(
        ...
        let height = i32::try_from(request.desired_height)
            .map_err(|_| sophia_runtime::ContentAllocationError::Malformed)?;
        let mut scale = (1, output.scale, 1);
        let (logical, pixel) = if request.role == 1 {
            ...
            scale = (
                parent.scale_generation,
                parent.scale_numerator,
                parent.scale_denominator,
            );
        ...
        Ok(ContentAllocationSnapshot {
            ...
            allocation,
            scale_generation: scale.0,
            scale_numerator: scale.1,
            scale_denominator: scale.2,
            role: request.role,
            edge: request.edge,

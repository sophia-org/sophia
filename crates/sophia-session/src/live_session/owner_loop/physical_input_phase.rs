    ...
    ($routing_mode:expr, $routed_input_coalescer:expr, $repaint_due:expr,
     $motion_held_since:expr, $frame_interval:expr) => {{
        ...
        if let Some(wm) = wm_session.as_mut() {
            let unavailable = wm.control_restart.is_some() || wm.degraded;
            if let Some(public) = wm.public.as_mut() {
                public.revalidate_presentation_input();
                if (unavailable || public.transport_unavailable || public.presentation_capture.exhausted())
        ...
        if let Some(poller) = physical_input.as_mut() {
            ...
            let (shortcuts, policy_presentation) = wm_session.as_mut().map_or((None, None), |wm| {
                let policy = wm.public.as_mut().map(|public| PolicyPresentedInputRouting {
                    state: &public.presentation_input,
                    capture: &mut public.presentation_capture,
                    protected_actions: public.actions.iter().filter(|action| action.session_operation_slot.is_some()).map(|action| action.action).collect(),
                });
                (wm.shortcuts.as_mut(), policy)
            });
            ...
            dispatch_physical_policy_inputs!(&report);

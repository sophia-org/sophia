...
def reserved_chord(context):
    ...
    with client(context) as observer, client(context) as injector:
        ...
        # The right-hand modifiers set the same bits, and the refusal is of
        # the bits: Control_R (105) and Alt_R (108) held, BackSpace is
        # refused the same way, and an ordinary key under them is delivered.
        fake(injector, opcode, 2, detail=105)
        injector.sync()
        key_event(observer, 2, window, key=105)
        fake(injector, opcode, 2, detail=108)
        injector.sync()
        key_event(observer, 2, window, key=108)
        fake(injector, opcode, 2, detail=22)
        injector.sync()
        observer.sync()
        no_input_yet(observer, window, .04)
        fake(injector, opcode, 2, detail=38)
        ...
        injector.sync()
        key_event(observer, 3, window, key=38)
        fake(injector, opcode, 3, detail=108)
        injector.sync()
        key_event(observer, 3, window, key=108)
        fake(injector, opcode, 3, detail=105)
        injector.sync()
        key_event(observer, 3, window, key=105)
        observer.sync()
        no_input_yet(observer, window, .04)
